...
Машина Colab временная, поэтому у каждого файла заранее определено место:
...
- Блокнот выполняется сверху вниз; любую ячейку можно перезапустить без вреда.
- Каждый блокнот начинается со «Старта сессии»: Drive, папки, переменные окружения, токены, код.
- Токены хранятся только в Colab Secrets (`HF_TOKEN`, `GITHUB_TOKEN`) и никогда не попадают в код.
